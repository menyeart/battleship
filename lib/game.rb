    ...
    player_shot = gets.chomp.upcase
      if @computer_board.valid_coordinate?(player_shot) == true
        if @computer_board.cells[(player_shot)].fired_upon? == false
          ...
          puts "\nThis cell has been fired upon previously, please enter a new coordinate"
          player_turn
        end
      else
  ...
  def result_report(board, shot_key, player)
    if board.cells[shot_key].empty? == true
      ...
      puts "\nThe #{player}'s shot on #{shot_key} was a hit!"
        if board.cells[shot_key].ship.sunk? == true
          puts "\nThe #{board.cells[shot_key].ship.name} has been sunk!"
        end
    end
  end
